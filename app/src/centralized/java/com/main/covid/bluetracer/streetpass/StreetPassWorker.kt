...
import com.main.covid.bluetracer.bluetooth.gatt.ACTION_DEVICE_PROCESSED
import com.main.covid.bluetracer.bluetooth.gatt.CONNECTION_DATA
import com.main.covid.bluetracer.bluetooth.gatt.DEVICE_ADDRESS
...
import com.main.covid.service.bluetooth.BluetoothMonitoringService.Companion.blacklistDuration
import com.main.covid.service.bluetooth.BluetoothMonitoringService.Companion.maxQueueTime
import com.main.covid.service.bluetooth.BluetoothMonitoringService.Companion.useBlacklist
...
//                || blacklist.filter { it.uniqueIdentifier == work.connectable.manuData }.isNotEmpty()
    ...
    fun finishWork(work: Work) {
        ...
        if (work.isCriticalsCompleted()) {
            BluetracerUtils.broadcastDeviceProcessed(context, work.device.address)
        ...
        override fun onCharacteristicRead(
                    ...
                    if (BlueTrace.supportsCharUUID(characteristic.uuid)) {
                        ...
                        try {
                            ...
                            connectionRecord?.let {
                                BluetracerUtils.broadcastStreetPassReceived(
                                    context,
                                    connectionRecord
                                )
                            }
            ...
            if (BlueTrace.supportsCharUUID(characteristic.uuid)) {
                ...
                if (TempIDManager.bmValid(context)) {
                    ...
                    val writedata = bluetraceImplementation.central.prepareWriteRequestData(
