...
import com.main.covid.utils.DP3TUploadData;
...
public class MainActivity extends AppCompatActivity implements NetworkStateReceiver.NetworkStateReceiverListener {
    ...
    private final int CAMERA_PERM = 1001;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!DP3T.isTracingEnabled(this))
            initializeRunningServices();

//        DP3TUploadData.INSTANCE.getExposureNotificationsConfig(this);
//        DataUploader.INSTANCE.downloadDP3TData(this);
//        setIAmInfectedDP3T();
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions,
        ...
        switch (requestCode) {
            ...
            case CAMERA_PERM:
                boolean isAllowedCamera = grantResults.length == 1 && grantResults[0] == PackageManager.PERMISSION_GRANTED;
                if (mRequest != null && isAllowedCamera)
                    mRequest.grant(new String[]{RESOURCE_VIDEO_CAPTURE});
                mRequest = null;
                break;
...
//            callback.invoke(origin, true, false);
        ...
        public void onPermissionRequest(PermissionRequest request) {
            if(ActivityCompat.checkSelfPermission(MainActivity.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED){
                ...
                request.grant(new String[]{RESOURCE_VIDEO_CAPTURE});
        }
    }
    ...
    public void startBTService() {
        ...
        if (!batteryOptDeact)
            startActivityForResult(new Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS,
                    Uri.parse("package:" + this.getPackageName())), REQUEST_IGNORE_BATTERY_OPTIMIZATION);
        else
            GaenStateHelper.checkGaenEnabled(this, gaenEnabled -> {
                if (gaenEnabled) {
                    continueDP3TInitialization();
                } else
                    checkGoogleServices();
            });
    }
    ...
    private void checkGoogleServices() {
        DP3T.checkGaenAvailability(this, gaenAvailability -> {
    ...
    private void continueDP3TInitialization() {

        DP3T.start(this,
                () -> {
//                    DP3TUploadData.INSTANCE.downloadDP3TData();
                    Toast.makeText(this, "Exposure notifications started", Toast.LENGTH_SHORT).show();
                },
                (e) -> Toast.makeText(this, "Exposure notifications failed: " + e.getClass().getSimpleName() + ": " + e.getMessage(),
                        Toast.LENGTH_SHORT).show(),
                () -> Toast.makeText(this, "Exposure notifications cancelled", Toast.LENGTH_SHORT).show());
    }
    ...
    public void setIAmInfectedDP3T() {

        if (firebaseAnalytics != null) {
            Bundle bundle = new Bundle();
            firebaseAnalytics.logEvent("user_infected", bundle);
        }

        DP3TUploadData.INSTANCE.uploadDP3TIAmInfected(this);
    }
...
//        3 -> Error de permisos de las EN
//        4 -> Notificaciones pushs no activadas
//        5 -> EN no disponibles en el dispositivo
        ...
        if (statusValue == 0) {
            ...
            if (status.isTracingEnabled()) {
                ...
                if (!errors.isEmpty()) {
                    ...
                    } else if (errors.contains(TracingStatus.ErrorState.GAEN_NOT_AVAILABLE)) {
                        checkGaenAvailable();
                    } else
                        checkGaenEnabled();
                } else
    ...
    private void checkGaenAvailable() {
        DP3T.checkGaenAvailability(this, gaenAvailability -> {
            boolean available = gaenAvailability == GaenAvailability.AVAILABLE;
            if (!available)
                sendStatusToWebView(5);
            else
                sendStatusToWebView(1);
        });
    }

    private void checkGaenEnabled() {
        GaenStateHelper.checkGaenEnabled(this, gaenEnabled -> {
            if (!gaenEnabled)
                sendStatusToWebView(3);
            else
                sendStatusToWebView(1);
        });
    }
