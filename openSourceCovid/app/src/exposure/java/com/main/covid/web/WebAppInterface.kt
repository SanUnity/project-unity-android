...
class WebAppInterface(
    ...
    fun postData(input: String) {
        ...
        if (!res.isNullOrEmpty() || res.isNotEmpty()) {
            ...
            token?.let {
                ...
                DP3TUploadData.getExposureNotificationsConfig(mContext)
                try {
    ...
    fun logout() {
        ...
        if (sharedPrefsRepository.getBTOn() && BuildConfig.BT_TYPE == Constants.DECENTRALIZED) {
            if (DP3T.isTracingEnabled(this.mContext))
                DP3T.stop(this.mContext)
    ...
    @JavascriptInterface
    fun getDeviceToken(input: String){
        val res = input.split(",").map { it.trim() }
        DataUploader.forceUploadDeviceToken(res)
    }
